...
public class ProductCommandService {
    ...
    ProductCommandService(ProductRepository productRepository, ApplicationEventPublisher eventPublisher) {
    ...
    public Product create(CreateProductCommand cmd) {
        ...
        var entity = new ProductEntity(
                cmd.code(), cmd.name(), cmd.description(), cmd.imageUrl(), cmd.price(), cmd.category());
    ...
    public Product update(String code, UpdateProductCommand cmd) {
        var entity = productRepository.findByCode(code).orElseThrow(() -> new ProductNotFoundException(code));
    ...
    private Product toProduct(ProductEntity e) {
        ...
        return new Product(
                e.getCode(), e.getName(), e.getDescription(), e.getImageUrl(), e.getPrice(), e.getCategory(), 0.0, 0);
    }
}
