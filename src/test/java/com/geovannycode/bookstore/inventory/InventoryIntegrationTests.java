...
class InventoryIntegrationTests {
    ...
    void shouldDecreaseStockWhenOrderCreatedEventReceived(Scenario scenario) {
        ...
        var event = new OrderCreatedEvent(
                "ORD-TEST-001",
                List.of(new OrderCreatedEvent.Item("P001", "Clean Code", new BigDecimal("45.99"), 3)),
                new OrderCreatedEvent.Customer(
                        "Test User", "test@test.com",
                        "+57 300 0000000", "Test Address"));

        scenario.publish(event)
                .andWaitForStateChange(() -> inventoryService.getStockLevel("P001"))
                .andVerify(newStockLevel -> assertThat(newStockLevel).isEqualTo(stockBefore - 3));
    }
        ...
        var event = new OrderCreatedEvent(
                ...
                List.of(
                        ...
                        new OrderCreatedEvent.Item(
                                "P003", "Designing Data-Intensive Applications", new BigDecimal("59.99"), 2)),
                new OrderCreatedEvent.Customer(
                        "Bulk Buyer", "bulk@test.com",
                        "+57 311 0000000", "Warehouse"));

        scenario.publish(event)
                .andWaitForStateChange(() -> inventoryService.getStockLevel("P002"))
                .andVerify(newStockP002 -> {
                    assertThat(newStockP002).isEqualTo(stockP002Before - 5);
                    assertThat(inventoryService.getStockLevel("P003")).isEqualTo(stockP003Before - 2);
                });
    }
